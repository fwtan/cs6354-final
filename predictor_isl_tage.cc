// Description: ISL-TAGE predictor for cbp3.
// Modified from the implementation: https://www.jilp.org/jwac-2/program/03_seznec.tgz
...
// Cyclic Shift Register for folding 
...
public:
    ...
    int bindex(uint32_t pc) { return ((pc) & ((1 << (LOGB)) - 1));}
    ...
    // a simple pseudo random number generator: use available information
    ...
    void FetchHistoryUpdate (uint32_t pc, uint16_t brtype, bool taken, uint32_t target)
    {
        if (brtype & IS_BR_CONDITIONAL)
        {
            #ifdef LOOPPREDICTOR
        ...
        HistoryUpdate(pc, brtype, taken, target, Fetch_phist, Fetch_ptghist, Fetch_ch_i, Fetch_ch_t[0], Fetch_ch_t[1]);
    }
    ...
    // PREDICTOR UPDATE
    void update_brcond(uint32_t pc, uint16_t brtype, bool taken, uint32_t target)
    {
        if (brtype & IS_BR_CONDITIONAL) 
        {
            PtIumRetire--;
        ...
        HistoryUpdate(pc, brtype, taken, target, Retire_phist, Retire_ptghist, Retire_ch_i, Retire_ch_t[0], Retire_ch_t[1]);
    }
...
void PredictorRunACycle ()
{
    ...
    for (int i = 0; i < cycle_info->num_fetch; i++)
    {
        ...
        if (uop->type & IS_BR_CONDITIONAL)
        {
            ...
            bool gpred = g_predictor->predict_brcond(uop->pc & 0x3ffff, uop->type);
            // report prediction:
        ...
        if (uop->type & 31)
        {
            ...
            g_predictor->FetchHistoryUpdate(uop->pc & 0x3ffff, uop->type, uop->br_taken, uop->br_target & 0x7f);
        } 
    }
    
    // update predictor at retire stage
    for (int i = 0; i < cycle_info->num_retire; i++)
    {
        ...
        if (uop->type & 31)
        {
            g_predictor->update_brcond(uop->pc & 0x3ffff, uop->type, uop->br_taken, uop->br_target & 0x7f);
        }
    }
}
